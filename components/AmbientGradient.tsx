...
const variantToStops: Record<
    AmbientVariant,
    { line: string; a: string; b: string; c: string }
> = {
    violet: {
...
export const palettes = {
    aurora: [
        "oklch(0.86 0.16 330 / 0.85)",
        "oklch(0.88 0.12 280 / 0.85)",
        "oklch(0.9 0.1 210 / 0.85)",
    ],
    lagoon: [
        "oklch(0.86 0.13 220 / 0.85)",
        "oklch(0.9 0.13 180 / 0.85)",
        "oklch(0.92 0.11 120 / 0.85)",
    ],
    candy: [
        "oklch(0.86 0.22 20 / 0.85)",
        "oklch(0.86 0.2 330 / 0.85)",
        "oklch(0.9 0.18 290 / 0.85)",
    ],
    sundown: [
        "oklch(0.84 0.23 30 / 0.85)",
        "oklch(0.88 0.18 350 / 0.85)",
        "oklch(0.86 0.16 310 / 0.85)",
    ],
    sunrise: [
        "oklch(0.95 0.22 85 / 0.85)",
        "oklch(0.96 0.18 55 / 0.85)",
        "oklch(0.97 0.1 20 / 0.85)",
    ],
    mint: [
        "oklch(0.94 0.12 150 / 0.85)",
        "oklch(0.96 0.1 120 / 0.85)",
        "oklch(0.97 0.08 90 / 0.85)",
    ],
    grape: [
        "oklch(0.88 0.18 300 / 0.85)",
        "oklch(0.86 0.16 320 / 0.85)",
        "oklch(0.9 0.12 260 / 0.85)",
    ],
    dusk: [
        "oklch(0.9 0.12 260 / 0.85)",
        "oklch(0.9 0.12 300 / 0.85)",
        "oklch(0.94 0.08 20 / 0.85)",
    ],
    peach: [
        "oklch(0.95 0.16 40 / 0.85)",
        "oklch(0.96 0.12 20 / 0.85)",
        "oklch(0.98 0.06 95 / 0.85)",
    ],
    aurum: [
        "oklch(0.95 0.18 90 / 0.85)",
        "oklch(0.96 0.14 80 / 0.85)",
        "oklch(0.98 0.08 70 / 0.85)",
    ],
} as const;
...
export function AmbientGradient({
    ...
    const lA = lA0,
        cA = Math.max(0, cA0 + jitter(h >>> 10, 6)),
        aA = Math.max(0.2, aA0 + jitter(h >>> 11, 8));
    const lB = lB0,
        cB = Math.max(0, cB0 + jitter(h >>> 12, 6)),
        aB = Math.max(0.18, aB0 + jitter(h >>> 13, 8));
    const lC = lC0,
        cC = Math.max(0, cC0 + jitter(h >>> 14, 6)),
        aC = Math.max(0.14, aC0 + jitter(h >>> 15, 8));
    const lD = lD0,
        cD = Math.max(0, cD0 + jitter(h >>> 18, 5)),
        aD = Math.max(0.12, aD0 + jitter(h >>> 19, 8));
    const lLine = lLine0,
        cLine = Math.max(0, cLine0 + jitter(h >>> 16, 6)),
        aLine = Math.max(0.15, aLine0 + jitter(h >>> 17, 10));
    ...
    const left4Pct = 20 + ((h >>> 9) % 60);
    const top4Pct = 15 + ((h >>> 10) % 55);
            ...
            <div className="absolute inset-0">
                <div
                    ...
                    style={{
                        ...
                        width: `${s1}px`,
                        height: `${s1}px`,
                        background: `radial-gradient(ellipse at center, oklch(${lA} ${cA} ${hueA} / ${aA}), transparent 70%)`,
                    }}
                />
                <div
                    className="absolute rounded-full blur-3xl"
                    style={{
                        ...
                        width: `${s2}px`,
                        height: `${s2}px`,
                        background: `radial-gradient(ellipse at center, oklch(${lB} ${cB} ${hueB} / ${aB}), transparent 70%)`,
                    }}
                />
                <div
                    ...
                    style={{
                        ...
                        width: `${s3}px`,
                        height: `${s3}px`,
                        background: `radial-gradient(ellipse at center, oklch(${lC} ${cC} ${hueC} / ${aC}), transparent 70%)`,
                    }}
                />
                <div
                    className="absolute rounded-full blur-3xl -translate-x-1/2"
                    style={{
                        ...
                        width: `${s4}px`,
                        height: `${s4}px`,
                        background: `radial-gradient(ellipse at center, oklch(${lD} ${cD} ${hueD} / ${aD}), transparent 72%)`,
                    }}
                />
            </div>
        </div>
